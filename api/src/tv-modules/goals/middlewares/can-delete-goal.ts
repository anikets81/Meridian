...
export const canDeleteGoal = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!permissions) {
        $logger.error('Can not get permissions for canDeleteGoal middleware');
        return res.status(500).end();
    }

    if (permissions.hasPermissions(GoalPermissions.GOAL_CAN_DELETE)) {
        return next();
    }
