...
import { GoalListArkTypeFetch } from '../list.types';

export const canFetchLists = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!permissions) {
        $logger.error('Can not get permissions for canFetchLists middleware');
        return res.status(500).end();
    }

    if (permissions.hasPermissions(GoalPermissions.GOAL_CAN_WATCH_CONTENT)) {
        return next();
    }
